...
Native Streamlit build, pastel edition — layout and content use Streamlit
widgets (columns, metrics, buttons, dataframes, plotly charts). A CSS
snippet sets the pastel color palette and card styling.
...
# ---------------------------------------------------------------
PAPER   = "#FBF6F4"   # warm blush-white background
INK     = "#75658A"   # dusty lavender – headings & primary text
MONSOON = "#8FD3C7"   # pastel mint — primary chart color
AMBER   = "#F6C89F"   # pastel apricot — secondary chart color
RED     = "#E7A9C0"   # pastel rose — alert/contrast chart color
SKY     = "#A9CBE8"   # pastel sky blue — tertiary accent
MUTED   = "#A89BB7"   # soft lavender-grey – captions & secondary text
...
<style>
    ...
    section[data-testid="stSidebar"] * {{ color: #F3EEF6 !important; }}
    ...
    section[data-testid="stSidebar"] button * {{ color: inherit !important; }}
    ...
    div.stButton > button[kind="primary"] {{
        background-color: {RED} !important; color: {INK} !important;
    }}
    div.stButton > button[kind="primary"]:hover {{ background-color: #DE8FAC !important; }}

    /* Soft rounded cards for bordered containers (verdict boxes, etc.) */
    ...
    [data-testid="stTable"] thead th {{
        background-color: #F3EAF4 !important; color: {INK} !important;
    }}

    .pastel-pill {{
        display: inline-block; padding: 3px 14px; border-radius: 999px;
        font-size: 12.5px; font-weight: 700; letter-spacing: .03em;
        background: #F3EAF4; color: {INK}; margin-bottom: 10px;
    }}
</style>
...
# ---------------------------------------------------------------
# TAB / NAV CONFIG  (Overview removed — intro page covers the summary)
# ---------------------------------------------------------------
TABS = [
...
# ---------------------------------------------------------------
def verdict(text):
    """Pastel 'short answer' callout using a native bordered container."""
    box = st.container(border=True)
    box.markdown(f'<span class="pastel-pill">💡 SHORT ANSWER</span>', unsafe_allow_html=True)
    box.write(text)
...
def bar(df, x, y, color=None, colors=None, horizontal=False, height=320, stacked=False):
    ...
    fig.update_xaxes(gridcolor="#F1E9EE")
    fig.update_yaxes(gridcolor="#F1E9EE")
    st.plotly_chart(fig, use_container_width=True)

def line(df, x, y, height=320):
    fig = px.line(df, x=x, y=y, markers=True)
    fig.update_traces(line_color=MONSOON, line_width=3, marker=dict(size=8, color=RED),
                       fill="tozeroy", fillcolor="rgba(143,211,199,0.20)")
    fig.update_layout(height=height, margin=dict(l=10, r=10, t=10, b=10),
                       plot_bgcolor="white", paper_bgcolor="white",
...
def render_cover():
    ...
    st.title("Mumbai Climate Disclosure Dashboard")
    st.write(
        "An interactive analysis of Mumbai's climate reporting trends from 2021 to 2025."
    )

    st.write("")
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Reporting Years", "5")
    c2.metric("Responses Analysed", "576")
    c3.metric("Key Analyses", "11")
    c4.metric("Climate Hazards", "4")
